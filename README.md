...
| Networks | Per-region list of every Neutron network with type-to-search by name / id / project. Each row has a caret that expands to show the DHCP and L3 agents hosting that network — equivalent to `openstack network agent list --network <id>`, but read straight from the regional DB. |
